...
def welch_PSD(epochs_eyes_closed, subject, participant):
    ...
    PD_psds_welch_mean, freqs_mean = psd_welch(epochs_eyes_closed, average='mean', **kwargs)
...
def mean_welch_psd(welch_subjs_mean, freqs_mean, participant):
    ...
    freq_mean_arr = freq
    np.save(f'/Users/senthilp/Desktop/mne_tutorial/scripts/data/{participant}_psd_mean_EC.npy', PSD_mean_arr)
    np.save(f'/Users/senthilp/Desktop/mne_tutorial/scripts/data/{participant}_freq_mean_EC.npy', freq_mean_arr)


def GFP(epochs_eyes_closed1, epochs_eyes_closed2, session):
    eyesOpen_S1 = epochs_eyes_closed1.average()
    eyesOpen_S2 = epochs_eyes_closed2.average()
...
# print(power_eyes_closed_avg1)
# #power_eyes_closed_avg.plot_topo(vmin=vmin, vmax=vmax, title='Using Morlet wavelets and EpochsTFR', show=True)

# mne.viz.plot_compare_evokeds(dict(auditory=power_eyes_closed_avg1, visual=power_eyes_closed_avg2),
